        ...
        <form method="POST" action="{{ url('games/'.$game->id) }}"
            ...
            <div class="form-group">
                <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                    value="{{ old('name', $game->name) }}" placeholder="Nombre" autofocus>
            ...
            <div class="form-group">
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" cols="30"
                    rows="4"
                    placeholder="Descripción">{{ old('description', $game->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <div class="custom-file">
                    <input type="file" class="custom-file-input @error('image') is-invalid @enderror" id="photo"
            ...
            <div class="form-group">
                <select name="user_id" class="form-control @error('user_id') is-invalid @enderror">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" @if ($user->id == old('user_id', $game->user_id)) selected
                            @endif>{{ $user->fullname }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <select name="category_id" class="form-control @error('category_id') is-invalid @enderror">
                    ...
                    @foreach($cats as $cat)
                        <option value="{{ $cat->id }}" @if ($cat->id == old('category_id', $game->category_id))
            ...
            <div class="form-group">
                <input id="price" type="number" class="form-control @error('price') is-invalid @enderror" name="price"
                    value="{{ old('price', $game->price) }}" placeholder="Precio" autofocus>
